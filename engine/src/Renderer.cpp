...
void Renderer::initVulkan() {
  ...
  window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Vulkan app", nullptr,
                            nullptr);
  assert(window);
...
void Renderer::createSwapchain(std::uint32_t width, std::uint32_t height) {
  ...
  vmaCreateImage(allocator, &imgCreateInfo, &imgAllocInfo, &drawImage.image,
...
void Renderer::initDescriptors() {
  ...
  drawImageDescriptors =
      descriptorAllocator.allocate(device, drawImageDescriptorLayout);
...
void Renderer::initBackgroundPipelines() {
  ...
  VK_CHECK(vkCreatePipelineLayout(device, &layout, nullptr,
  ...
  VK_CHECK(vkCreateComputePipelines(device, VK_NULL_HANDLE, 1,
                                    &pipelineCreateInfo, 0, &gradientPipeline));
...
void Renderer::initTrianglePipeline() {
  ...
  assert(vkutil::loadShaderModule("shaders/colored_triangle.vert.spv", device,
                                  &vertexShader));
  VkShaderModule fragShader;
  assert(vkutil::loadShaderModule("shaders/colored_triangle.frag.spv", device,
                                  &fragShader));
  ...
  trianglePipeline =
      PipelineBuilder{trianglePipelineLayout}
...
void Renderer::initImGui() {
  { // init Vulkan structures
    const auto poolCreateInfo = vkinit::commandPoolCreateInfo(
        VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, graphicsQueueFamily);
    VK_CHECK(vkCreateCommandPool(device, &poolCreateInfo, nullptr,
                                 &imguiCommandPool));
    const auto cmdAllocInfo =
        vkinit::commandBufferAllocateInfo(imguiCommandPool, 1);
    VK_CHECK(
        vkAllocateCommandBuffers(device, &cmdAllocInfo, &imguiCommandBuffer));
    ...
    deletionQueue.pushFunction(
        [this]() { vkDestroyFence(device, imguiFence, nullptr); });
  }
...
void Renderer::update(float dt) {
  ...
  auto glmToArr = [](const glm::vec4 &v) {
    return std::array<float, 4>{v.x, v.y, v.z, v.w};
  };
...
void Renderer::draw() {
  ...
  VK_CHECK(
      vkWaitForFences(device, 1, &currentFrame.renderFence, true, NO_TIMEOUT));
  ...
  const auto cmdBeginInfo = vkinit::commandBufferBeginInfo(
  ...
  vkutil::transitionImage(cmd, drawImage.image, VK_IMAGE_LAYOUT_GENERAL,
                          VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);

  drawGeometry(cmd);

  vkutil::transitionImage(cmd, drawImage.image,
                          VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                          VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);

  // get swapchain image
  std::uint32_t swapchainImageIndex{};
  VK_CHECK(vkAcquireNextImageKHR(device, swapchain, NO_TIMEOUT,
                                 currentFrame.swapchainSemaphore,
                                 VK_NULL_HANDLE, &swapchainImageIndex));
  auto &swapchainImage = swapchainImages[swapchainImageIndex];
  // copy from draw image
  vkutil::transitionImage(cmd, swapchainImage, VK_IMAGE_LAYOUT_UNDEFINED,
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
  vkutil::copyImageToImage(cmd, drawImage.image, swapchainImage, drawExtent,
                           swapchainExtent);
  vkutil::transitionImage(cmd, swapchainImage,
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                          VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
  ...
  { // submit
    ...
    auto waitInfo = vkinit::semaphoreSubmitInfo(
        ...
        currentFrame.swapchainSemaphore);
    auto signalInfo = vkinit::semaphoreSubmitInfo(
...
void Renderer::drawBackground(VkCommandBuffer cmd) {
  ...
  vkCmdClearColorImage(
      cmd, drawImage.image, VK_IMAGE_LAYOUT_GENERAL, &clearValue, 1,
  &clearRange); */
  ...
  vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                          ...
                          0, nullptr);
  vkCmdPushConstants(cmd, gradientPipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT,
                     0, sizeof(ComputePushConstants), &gradientConstants);
  vkCmdDispatch(cmd, std::ceil(drawExtent.width / 16.f),
                std::ceil(drawExtent.height / 16.f), 1.f);
}
...
void Renderer::drawGeometry(VkCommandBuffer cmd) {
  ...
  const auto renderInfo =
      vkinit::renderingInfo(drawExtent, &colorAttachment, nullptr);
...
void Renderer::drawImGui(VkCommandBuffer cmd, VkImageView targetImageView) {
  ...
  const auto renderInfo =
      vkinit::renderingInfo(swapchainExtent, &colorAttachment, nullptr);
  vkCmdBeginRendering(cmd, &renderInfo);
