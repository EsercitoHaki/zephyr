...
template <typename T>
std::span<const T> getPackedBufferSpan(const tinygltf::Model &model,
  ...
  const auto *data = reinterpret_cast<const T *>(
      &buf.data.at(0) + bv.byteOffset + accessor.byteOffset);
  return std::span{data, accessor.count};
}

int findAttributeAccessor(const tinygltf::Primitive &primitive,
...
bool hasAccessor(const tinygltf::Primitive &primitive,
                 const std::string &attributeName) {
  const auto accessorIndex = findAttributeAccessor(primitive, attributeName);
...
std::filesystem::path
getDiffuseTexturePath(const tinygltf::Model &model,
...
void loadPrimitive(const tinygltf::Model &model, const std::string &meshName,
  ...
  if (primitive.indices != -1) { // load indices
    ...
    const auto indices =
        getPackedBufferSpan<std::uint16_t>(model, indexAccessor);
  ...
  { // get min and max pos
    const auto posAccessorIndex =
        findAttributeAccessor(primitive, GLTF_POSITIONS_ACCESSOR);
  ...
  if (hasAccessor(primitive, GLTF_TANGENTS_ACCESSOR)) {
    const auto tangents = getPackedBufferSpan<glm::vec4>(
  ...
  if (hasAccessor(primitive, GLTF_JOINTS_ACCESSOR)) {
    ...
    const auto joints = getPackedBufferSpan<std::uint8_t[4]>(
        model, primitive, GLTF_JOINTS_ACCESSOR);
    const auto weights =
        getPackedBufferSpan<float[4]>(model, primitive, GLTF_WEIGHTS_ACCESSOR);
...
void loadMaterial(const util::LoadContext &ctx, Material &material,
...
bool shouldSkipNode(const tinygltf::Node &node) {
  ...
  if (node.name.starts_with("Collision") || node.name.starts_with("Trigger") ||
      node.name.starts_with("PlayerSpawn") ||
      node.name.starts_with("Interact")) {
    return true;
  }
...
void loadNode(SceneNode &node, const tinygltf::Node &gltfNode,
  ...
  for (std::size_t childIdx = 0; childIdx < gltfNode.children.size();
       ++childIdx) {
...
Skeleton loadSkeleton(std::unordered_map<int, JointId> &gltfNodeIdxToJointId,
                      const tinygltf::Model &model,
...
std::unordered_map<std::string, SkeletalAnimation>
loadAnimations(const Skeleton &skeleton,
  ...
  for (const auto &gltfAnimation : gltfModel.animations) {
    ...
    for (const auto &channel : gltfAnimation.channels) {
      ...
      animation.duration = static_cast<float>(timesAccessor.maxValues[0] -
                                              timesAccessor.minValues[0]);
      ...
      if (channel.target_path == GLTF_SAMPLER_PATH_TRANSLATION) {
        ...
        if (translationKeys.size() == 2 &&
            translationKeys[0] == translationKeys[1]) {
        ...
        const auto rotationKeys =
            getPackedBufferSpan<glm::vec4>(gltfModel, outputAccessor);
...
namespace util {
void SceneLoader::loadScene(const LoadContext &ctx, Scene &scene,
  ...
  // load materials
  for (std::size_t materialIdx = 0; materialIdx < gltfModel.materials.size();
       ++materialIdx) {
  ...
  for (const auto &gltfMesh : gltfModel.meshes) {
    ...
    for (std::size_t primitiveIdx = 0;
         primitiveIdx < gltfMesh.primitives.size(); ++primitiveIdx) {
  ...
  for (const auto &skin : gltfModel.skins) {
    scene.skeletons.push_back(
        loadSkeleton(gltfNodeIdxToJointId, gltfModel, skin));
  }
  ...
  if (!gltfModel.skins.empty()) {
    ...
    scene.animations =
        loadAnimations(scene.skeletons[0], gltfNodeIdxToJointId, gltfModel);
  }
